from task1_booking import kpop_concert_booking
from task2_addons import select_ticket_and_addons
from task3_approval import approve_booking

def display_concert_booking():
    id_counter = 5000  # Starting ticket ID for the tour

    id_counter, ticket_id, id_number, fan_id, fan_name, tour_city = kpop_concert_booking(id_counter)
    selected_options, total_cost = select_ticket_and_addons()
    status, ref_number = approve_booking(total_cost, ticket_id, id_number)

    print("\n YOUR K-POP CONCERT BOOKING RECEIPT")
